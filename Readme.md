...
- No shipped ecosystem profile or provider/model pair has passed the required certification benchmark. Preview-generated runs cannot become `VERIFIED`, so apply/rollback are implemented safety paths but are not enabled for normal generated runs yet.
