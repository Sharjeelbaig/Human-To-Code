...
Enable it in config or with `--compiler`. The default deterministic rule layer
runs locally and sends no provider request. `compiler.onUnderspecified: "error"`
exits 3 before confirmation or writes; `"warn"` reports the questions and
continues. `--explain-spec` shows which required facets were satisfied.
...
the instruction plus required inline source/FileMemory context. It skips turn
classification, ProjectMemory, selected model skills, shared blueprints, todo
planning, cross-file audits, and integration reconciliation—even when those
`direct` features are enabled. Deterministic candidate validation still runs,
and one failed candidate may receive one bounded correction attempt before the
unit fails closed. Normal mode keeps the full project-aware agent workflow.
